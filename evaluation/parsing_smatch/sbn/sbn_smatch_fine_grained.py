...
def normalize_gold_line(line: str) -> str:
    """
    gold 文件：
    - 支持纯 SBN
    - 支持 id\\tSBN
    - 去掉首尾空白
    - 不主动删 <drs>，因为 gold 正常不该带
    """
    line = line.strip()
    if not line:
        return ""
...
def normalize_pred_line(line: str) -> str:
    """
    pred 文件：
    - 默认一行纯 SBN
    - 去掉首尾空白
    ...
    if line.startswith("<drs>"):
        line = line[len("<drs>"):].strip()
...
def create_arg_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="gold sbn file path, one sbn per line"
    )
    parser.add_argument(
        "-s2", "--sbn_file2",
        required=True,
        type=str,
        help="predicted sbn file path, one sbn per line"
    )
    parser.add_argument(
        "-e", "--evaluation",
        default="smatch",
        type=str,
        choices=["smatch", "node", "triple"],
        help="evaluation type: smatch / node / triple"
    )
    parser.add_argument(
        "-d", "--detail",
        default="none",
        type=str,
        choices=["none", "role", "relation", "operator", "sense"],
        help="fine-grained detail mode for smatch"
    )
    parser.add_argument(
        "-f", "--fix_ill",
        default=True,
        type=str2bool,
        help="whether to iteratively truncate ill-formed prediction"
    )
...
if __name__ == '__main__':
    ...
    with open(sbn_path, "r", encoding="utf-8") as f:
        sbn_data_raw = f.readlines()

    with open(sbn_path2, "r", encoding="utf-8") as f2:
        sbn_data2_raw = f2.readlines()

    sbn_data = [normalize_gold_line(x) for x in sbn_data_raw]
    sbn_data2 = [normalize_pred_line(x) for x in sbn_data2_raw]

    if len(sbn_data) != len(sbn_data2):
        print(f"[Warn] line count mismatch: gold={len(sbn_data)} pred={len(sbn_data2)}")

    n = min(len(sbn_data), len(sbn_data2))
    print(f"[Info] evaluating {n} line pairs")
    print(f"[Info] evaluation={evaluation}, detail={detail}, fix_ill={fix_ill}")
    ...
    c2c_pred, c2c_gold, c2n_pred, c2n_gold = [], [], [], []
    b2c_pred, b2c_gold, c2o_pred, c2o_gold = [], [], [], []
    b2b_pred, b2b_gold = [], []
    ...
    for i in range(n):
        ...
        if not sbn1:
            print(f"[Skip] gold line {i} is empty")
            gold_error += 1
            continue

        if not sbn2:
            print(f"[Skip] pred line {i} is empty")
            pred_error += 1
            continue

        # ---------- gold -> penman ----------
        try:
            sbn_graph_1 = SBNGraph().from_string(sbn1, is_single_line=True)
            penman1 = sbn_graph_1.to_penman_string()
        except Exception as e:
            print(f"[Gold Error] line {i}: {e}")
            print(f"  gold: {sbn1}")
            gold_error += 1
            continue
        ...
        if not fix_ill:
            try:
                sbn_graph_2 = SBNGraph().from_string(sbn2, is_single_line=True)
                penman2 = sbn_graph_2.to_penman_string()
            except Exception as e:
                print(f"[Pred Error] line {i}: {e}")
                print(f"  pred: {sbn2}")
                pred_error += 1
                ill_form += 1
                continue
            ...
            while length > 0:
                try:
                    sbn2_try = " ".join(sbn2_split[:length])
                    sbn_graph_2 = SBNGraph().from_string(sbn2_try, is_single_line=True)
                    penman2 = sbn_graph_2.to_penman_string()
                    if length < len(sbn2_split):
                        ill_form += 1
                    break
                except Exception:
                    length -= 1
            ...
            if penman2 is None:
                print(f"[Pred Fatal Error] line {i}: unable to repair prediction")
                print(f"  pred: {sbn2}")
                pred_error += 1
                ill_form += 1
                continue
        ...
        try:
            penman1 = penman1.replace("\n", " ")
            penman2 = penman2.replace("\n", " ")

            if evaluation == "smatch":
                penman1_fg = penman_fine_grained(penman1, detail)
                penman2_fg = penman_fine_grained(penman2, detail)

                for (precision, recall, best_f_score) in score_amr_pairs(
                    [penman1_fg], [penman2_fg], remove_top=True
                ):
                    print(f"{i}: {best_f_score:.6f}")
            ...
            print(f"[Score Error] line {i}: {e}")

    print("\n========== SUMMARY ==========")
    print(f"[Info] valid_pairs      = {valid_pairs}")
    print(f"[Info] gold_errors      = {gold_error}")
    print(f"[Info] pred_errors      = {pred_error}")
    print(f"[Info] ill_formed_count = {ill_form}")
    print(f"[Info] ill_formed_ratio = {ill_form / n if n > 0 else 0:.6f}")

    if evaluation == "smatch":
        denom = valid_pairs if valid_pairs > 0 else 1
        print(f"[Result] average f1 smatch score under '{detail}' evaluation: {average_f1 / denom:.6f}")

    elif evaluation == "node":
        print("[Result] node-level scores")
        for score in preds:
            pr = inters[score] / float(preds[score]) if preds[score] > 0 else 0.0
            rc = inters[score] / float(golds[score]) if golds[score] > 0 else 0.0
            f = 2 * (pr * rc) / (pr + rc) if (pr + rc) > 0 else 0.0
            print(f"{score} -> P: {pr:.3f}, R: {rc:.3f}, F: {f:.3f}")

    elif evaluation == "triple":
        print("[Result] triple-level scores")

        pr, rc, f = smatch_fromlists.main(c2c_pred, c2c_gold, True)
        print(f"Roles_triple      -> P: {float(pr):.3f}, R: {float(rc):.3f}, F: {float(f):.3f}")

        pr, rc, f = smatch_fromlists.main(c2n_pred, c2n_gold, True)
        print(f"Names_triple      -> P: {float(pr):.3f}, R: {float(rc):.3f}, F: {float(f):.3f}")

        pr, rc, f = smatch_fromlists.main(b2c_pred, b2c_gold, True)
        print(f"Members_triple    -> P: {float(pr):.3f}, R: {float(rc):.3f}, F: {float(f):.3f}")

        pr, rc, f = smatch_fromlists.main(c2o_pred, c2o_gold, True)
        print(f"Operators_triple  -> P: {float(pr):.3f}, R: {float(rc):.3f}, F: {float(f):.3f}")

        pr, rc, f = smatch_fromlists.main(b2b_pred, b2b_gold, True)
        print(f"Discourses_triple -> P: {float(pr):.3f}, R: {float(rc):.3f}, F: {float(f):.3f}")